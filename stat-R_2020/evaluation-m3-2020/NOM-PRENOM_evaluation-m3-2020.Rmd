...
On the IFB-core-cluster RStudio server, the data is in the directory : `/shared/projects/dubii2020/data/module3/seance7/BIC`

If you work on a local instance, you can download a clone of this course with
...
Or just the files you need with

```{r download_data eval=FALSE}

dataFiles <- c("BIC_log2-norm-counts_edgeR_DEG_top_1000.tsv.gz","BIC_edgeR_DEG_table.tsv.gz","BIC_sample-classes.tsv.gz")
localDataDir <- "/shared/projects/dubii2020/trainers/github_clones/module-3-Stat-R/stat-R_2020/data/TCGA_BIC_subset"  # Adapt to yours
for (dataFile in dataFiles) {
  localFile <- file.path(localDataDir, dataFile)
  if (file.exists(localFile)) {
    ...
    message("Downloading data files from github: ", githubURL)
...
1. Statistique exploratoire: dessiner quelques figures permettant d'appréhender la distribution des données. 

    - histogramme des données
    - projection des échantillons sur les 6 premières composantes (3 graphiques avec PC1-2, PC3-4, et PC5-6)
...
The data were loaded from the folder `r data.folder`. 
...
BIC.expr <- read.table(file = file.path(data.folder, dataFiles["expression"]), header = TRUE)
# dim(BIC.expr)
BIC.sample.classes <- read.table(file.path(data.folder, dataFiles["sample classes"]),header = TRUE)
BIC.deg <- read.table(file.path(data.folder, dataFiles["DEG table"]),header = TRUE)
...
```{r top_deg}
## Sort the expression table by decreasing value of expression
BIC.deg.filtered <- subset(BIC.deg, row.names(BIC.deg) %in% row.names(BIC.expr.filtered))
BIC.expr.filtered.merged <- merge(BIC.expr.filtered, BIC.deg.filtered, by=0)
BIC.expr.filtered.sorted <- BIC.expr.filtered.merged[order(BIC.expr.filtered.merged$padj), ]
...
Run hierarchical clustering on the 
...
<!-- ```{r reduce_types} -->
<!-- new.type <- rep("Luminal.A", length = nrow(BIC.sample.classes.filtered)) -->
<!-- ``` -->

<!-- b. Identifiez les individus qui ne contiennent pas "Luminal.A" dans la première colonne (colonne `cancer.type`) du data.frame `BIC.sample.classes.filtered`.   -->

<!-- ```{r noLA_ind} -->
<!-- ind.noLA <- which(BIC.sample.classes.filtered[,"cancer.type"]!="Luminal.A") -->
<!-- ``` -->
...
<!-- BIC.sample.classes.2 <- BIC.sample.classes.filtered -->
...
<!-- * Calculez la matrice de corrélation entre les gènes différentiellement exprimés. Comme dans le data.frame `BIC.expr.filtered` les gènes sont en lignes, pensez à transposer votre data.frame. -->

<!-- ```{r correlation_matrix} -->
<!-- mat.cor <- cor(t(BIC.expr.filtered)) -->
<!-- dim(mat.cor) -->
...
<!-- * Supprimez ces gènes du data.frame `BIC.expr.filtered`. -->
<!-- ```{r suppress_correlated} -->
<!-- BIC.expr.filtered <- BIC.expr.filtered[-var.supp,] -->
<!-- dim(BIC.expr.filtered) -->
<!-- ``` -->
...
<!-- ```{r dataFrame} -->
<!-- df.data <- data.frame(BIC.sample.classes.2[,1], t(BIC.expr.filtered)) -->
<!-- dim(df.data) -->
<!-- ``` -->
...
# Preparation of the learning and testing sample sets
...
Créez un nouveau data.frame (`df.data.4`) qui contient les données d'expression de gènes contenu dans le data.frame `BIC.expr.filtered` et le type de cancer pour chaque échantillon (définit en 4 types).
 
```{r df_four_types}
df.data.4 <- data.frame(as.character(BIC.sample.classes.filtered[,1]), t(BIC.expr.filtered))
