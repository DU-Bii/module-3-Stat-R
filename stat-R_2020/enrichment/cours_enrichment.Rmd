...
localt4Data <- file.path(outdirs$data, t4_geneIDsFile)

if (file.exists(localt4Data) & !parameters$forceDownload) {
  message("t4 data already downloaded:\n\t", localt4Data)
} else {
  message("Downloading t4 data from github: ", t4URL)
  download.file(url = t4URL, destfile =  localt4Data)
}
```

# DEG in vector

```{r deg, include=FALSE, echo=FALSE, eval=TRUE}
t4degFile <- file.path(
  outdirs$data,
  "bdf1_Y187F_Y354F_mutant_4__vs__Wild_type_4_DESeq2_positive_geneIDs.txt")
t4degValues <- scan(file = t4degFile,
                    what = "character"
                    )
```
...
```{r gprofiler, include=FALSE, echo=FALSE, eval=TRUE}
...
```{r gprofplot, include=FALSE, echo=FALSE, eval=TRUE}
...
```{r genesets, include=FALSE, echo=FALSE, eval=TRUE}
...
```{r download_deg, include=FALSE, echo=FALSE, eval=TRUE}
#### Download DEG data ####
t0dataFile <- "bdf1_Y187F_Y354F_mutant_0__vs__Wild_type_0_DESeq2_positive.tsv"
t4dataFile <- "bdf1_Y187F_Y354F_mutant_4__vs__Wild_type_4_DESeq2_positive.tsv"
t0URL <- file.path("https://raw.githubusercontent.com/DU-Bii/module-3-Stat-R/master/stat-R_2019/seance_5/results", t0dataFile)
t4URL <- file.path("https://raw.githubusercontent.com/DU-Bii/module-3-Stat-R/master/stat-R_2019/seance_5/results", t4dataFile)
localt0Data <- file.path(outdirs$data, t0dataFile)
localt4Data <- file.path(outdirs$data, t4dataFile)

if (file.exists(localt0Data) & !parameters$forceDownload) {
  message("t0 data already downloaded:\n\t", localt0Data)
} else {
  message("Downloading t0 data from github: ", t0URL)
  download.file(url = t0URL, destfile =  localt0Data)
}
...
t0degFile <- file.path(
  outdirs$data,
  "bdf1_Y187F_Y354F_mutant_0__vs__Wild_type_0_DESeq2_positive.tsv")
t0degValues <- read.csv(file = t0degFile, header = 1, row.names = 1)
t4degFile <- file.path(
  outdirs$data,
  "bdf1_Y187F_Y354F_mutant_4__vs__Wild_type_4_DESeq2_positive.tsv")
t4degValues <- read.csv(file = t4degFile, header = 1, row.names = 1)
```

# Analysis

```{r gsea_analysis, include=FALSE, echo=FALSE, eval=TRUE}
