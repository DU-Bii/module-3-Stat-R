...
si vous travaillez sur une installation locale, vous pouvez les téléca
...
```{r}
plot.new() # To do: check why we need to add this
plot(pca.res, habillage = "ind", col.hab = vcol.set, label = "none")
legend("topleft", 
       legend = c("jeu d'apprentissage", "jeu test"), 
...
```{r}
kable(rf.fit, caption = "Confusion table obtained by Random Forest on the training set (learning errors).  ")
```
...
```{r fig.width=7, fig.height=5, out.width="60%", fig.cap="Random forest: impact of the number of tree on the misclassification error rate (MER)"}
