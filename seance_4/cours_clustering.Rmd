...
(*) Espace Euclidien à $n$ dimensions, où chaque dimension représente une des variables observées. Chaque individu est décrit comme un vecteur à $n$ valeurs, qui correspont à un point dans cet espace. 
...
```{r plot_iris, fig.width=7, fig.height=6, out.width="70%"}
plot(mes.iris)
```
...
- **Single linkage** : distance entre élements les plus proches des 2 groupes respectifs

$$D(C_1,C_2) = \min_{i \in C_1, j \in C_2} D(x_i, x_j)$$
  
- **Complete linkage** : distance entre éléments les plus éloignés des 2 groupes

$$D(C_1,C_2) = \max_{i \in C_1, j \in C_2} D(x_i, x_j)$$
  
- **Group average** : distance moyenne entre éléments des 2 groupes
...
```{r group_distances, echo=FALSE, out.width="50%", fig.cap="Distances entre groupes"}
...
species.pch <- c(setosa = "s", virginica = "v", versicolor = "e")
plot(mes.iris, col = species.colors[iris$Species], pch = species.pch[iris$Species], cex = 0.7)
```
...
```{r plot_raw_vs_scaled, out.width="80%", fig.width=12, fig.height=6}
par(mfrow = c(1,2))
plot(mes.iris, main = "Raw variables")
# plot(mes.iris.scaled, main = "Scaled variables")
par(mfrow = c(1,1))
```
...
```{r boxplots_raw_vs_norm, out.width="95%", fig.width=16, fig.height=6}
par(mfrow = c(1,2))
par(mar = c(5.1, 10, 4.1, 1.1)) # adapt margin sizes for the labels
boxplot(mes.iris, main = "Raw data", horizontal = TRUE, las = 1)
boxplot(mes.iris.scaled, main = "scaled")
...
```{r,  out.width="80%", fig.width=12, fig.height=6}
par(mfrow = c(1,2))
...
```{r, out.width="60%", fig.width=12, fig.height=6}
