...
1. A l'IFB : sur le serveur IFB-core-cluster, les données sont dans le répertoire

```
/shared/projects/du_bii_2019/data/module3/seance4/
```
  
Vous pouvez donc les lire à l'aide du code suivant (cliquer sur ***Code*** pour l'afficher).
...
```
  ...
  2. En local : en local, les données sont dans le répertoire `../../dubii/data-m3/s4`. Vous pouvez donc les lire à l'aide de la commande : `BIC.expr <- read.table("../../dubii/data-m3/s4/BIC_diff_exp.tsv", h=T)`
