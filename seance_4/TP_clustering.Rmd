...
```{r heatmap_genes_standardized, out.width="80%", fig.width=10, fig.height=10, fig.cap="Heat map of the expression matrix clustered by genes (rows) and samples (columns). Genes were centered on the mean in order to highlight over- and under-expression"} 
...
heatmap(as.matrix(BIC.expr.genes.standardized),
        distfun = function(x) as.dist(1 - cor(t(x), method = "spearman")), 
        hclustfun = function(x) hclust(x, method = "ward.D2"),
        labRow = NA, labCol = NA, col = rainbow(n = 100),
        scale = "none")
```
