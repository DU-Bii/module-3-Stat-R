...
## Liste des fichiers de données
data.folder <- "../../dubii/data-m3/s4/BIC/"
```
...
## Transformation ACP
BIC.pca <- prcomp(BIC.expr.transposed, center = FALSE, scale. = FALSE)
names(BIC.pca) # check the name of the fields of the PCA object

## Plot de l'écart-type sur les premières composantes
barplot(BIC.pca$sdev[1:10], main = "Standard deviation per PC")

## Visualisation des données
plot(BIC.pca, main = "Variance per PC")

```

<!--
```{r echo=F, results=F, eval=F}
save(BIC.pca, file="data_R4/BIC.pca.RData")
# Réduction des données, anova
dt2 <- data.frame(dt1, BIC.sample.classes[,1])
...
```{r, echo=F, eval=F}
# plot(BIC.hclust.ward,
#     labels=BIC.sample.classes$cancer.type, hang=0)
# plot(BIC.hclust.ward,
     # labels=BIC.sample.classes$xml_lab_proc_her2_neu_immunohistochemistry_receptor_status, hang=0, cex=0.5, las = 0)

ClassDiscovery::plotColoredClusters(
  BIC.hclust.ward, 
  labs=BIC.sample.classes$cancer.type, 
  cols=sample.colors, cex=0.5)

legend("topright",
       legend=sort(classes), 
       col = class.colors,
       text.col = class.colors, cex=0.5)
```
...
```{r}
BIC.kmeans <- kmeans(dt1, centers=20)
...
par(mfrow=c(1,2))
BIC.kmeans10 <- kmeans(dt1, centers=10)
table(BIC.kmeans10$cluster)
plot(BIC.pca, choix="ind", label="none", col.ind=BIC.kmeans10$cluster)
BIC.kmeans3 <- kmeans(dt1, centers=3)
table(BIC.kmeans3$cluster)
plot(BIC.pca, choix="ind", label="none", col.ind=BIC.kmeans3$cluster)
par(mfrow=c(1,1))
BIC.kmeans2 <- kmeans(dt1, centers=2)
table(BIC.kmeans2$cluster)
plot(BIC.pca, choix="ind", label="none", col.ind=BIC.kmeans2$cluster)
```
...
pointsToPlot <- BIC.pca$ind$coord[,1:2]
plot(BIC.pca, choix="ind", label="none", col.ind="white")
