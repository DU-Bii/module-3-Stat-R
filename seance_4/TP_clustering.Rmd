...
<!--
```{r}
save(BIC.prcomp, file="data_R4/BIC.prcomp.RData")
# Réduction des données, anova
dt2 <- data.frame(BIC.expr.transposed, BIC.sample.classes[,1])
p.value <- numeric(length=ncol(BIC.expr.transposed))
for (i in 1:ncol(BIC.expr.transposed)) {
...
BIC.all <- BIC.expr.transposed
BIC.expr.transposed <- BIC.expr.transposed[, which(p.fdr < 0.05)]
write.table(BIC.expr.transposed, "data_R4/BIC_diff_exp.tsv", sep="\t")
```
-->
