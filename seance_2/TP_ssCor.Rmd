...
<span style="color: #FF0000"> dire où se trouve le fichier de données </span>
...
1. Ouvrez un terminal et placez vous dans votre répertoire de travail.

2. Lancez le logiciel `Rstudio`. Ouvrez un nouveau fichier `Rmarkdown` que vous sauvegardez dans votre répertoire de travail.
...
apply(metaFile2[, var.sel],2,mean, na.rm=T)
apply(metaFile2[, var.sel],2,sd, na.rm=T)
```
...
* Est-ce que l'indice de masse corporel des patients est significativement plus grand de celui des contrôles sains ?
* Le taux d’albumine des patients est-il significativement différent de celui des contrôles sains ?
* Le taux de créatinine des patients est-il significativement différent de celui des contrôles sains ?
* Le taux de bilirubine des patients est-il significativement différent de celui des contrôles sains ?
...
```{r}
metaHeath <- metaFile2[which(metaFile2[,"status"]=="healthy"),]
dim(metaHeath)

metaLiver <- metaFile2[which(metaFile2[,"status"]=="liver"),]
dim(metaLiver)
```
...
Ensuite, on se demande si la maladie cyrrhose touche autant les hommes que les femmes. Pour répondre à cette question, vous allez réaliser un test du Chi2 d'homogénéité. 
...
```{r}
tcStG <- table(metaFile[,"status"], metaFile[,"Gender"])
```
...
```{r}
tcStE <- table(metaFile[,"status"], metaFile[,"Enterotype"])
res.chisq <-chisq.test(tcStE)
```
...
On se demande si la composition de l'entérotype modifie l'indice de masse corporelle des individus. 
...
```{r}
boxplot(BMI~Enterotype, data=metaFile2)
```
...
* Il faut que la variable aléatoire suive une loi normale dans les trois échantillons.
* Il faut que la variable aléatoire ait la même variance dans les trois échantillons (Homoscédaticité de $X$).  

3. Testez la normalité de $X$ dans les 3 échantillons. (fonction `by()`, `shapiro.test()`) 
```{r}
by(metaFile2[,"BMI"], metaFile2[,"Enterotype"], shapiro.test)
```


4. Testez l'homoscédaticité de $X$. (foncion `bartlett.test()`)
```{r}
bartlett.test(BMI~Enterotype, data=metaFile2)
```
...
```{r}
kruskal.test(BMI~Enterotype, data=metaFile2)
```
