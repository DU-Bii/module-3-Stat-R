...
```{r expr_histo, fig.width=7, fig.height=5, out.width="60%"}
hist(exprs$WT1)
```
...
```{r expr_log2_hist, fig.width=7, fig.height=5, out.width="60%"}
...
```{r expr_log2_boxplot, fig.width=7, fig.height=5, out.width="60%"}
...
```{r expr_log2_boxplot_horiz, fig.width=7, fig.height=5, out.width="60%"}
boxplot(log(exprs[, c("WT1", "WT2", "KO1", "KO2")]),
     main = "Expression boxplots",
     xlab = "log2(counts)", # X label
...
```{r xy_plot, fig.width=7, fig.height=5, out.width="60%"}
...
```{r genes_per_chrom_barplot, fig.width = 5, fig.height = 7, out.width = "40%"}
...
Pour vous approprier les commandes présentées ci-dessus, nous vous proposons d'analyser un fichier d'expression complet et de générer différentes représentations graphiques pour acquérir une intitution de la distribution des donnnées.
