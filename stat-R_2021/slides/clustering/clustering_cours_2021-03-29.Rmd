...
BIC_expr_file <- file.path(
  data.folder,
  "BIC_log2-norm-counts_edgeR_DEG_top_1000.tsv.gz")

## Load expression table
message("Loading expression file\t", BIC_expr_file)
BIC_expr <- read.table(file = BIC_expr_file, header = TRUE)
message("Loaded BIC expression table with ", nrow(BIC_expr), " rows (genes) x ", ncol(BIC_expr), " columns (samples)")
...
rownames(BIC_expr) <- sapply(rownames(BIC_expr), reduc_name)
colnames(BIC_expr) <- sapply(colnames(BIC_expr), reduc_name)
```
...
```{r data1, echo = FALSE}
kable(BIC_expr[1:6, 1:4])
```

Pour des raisons historiques, en analyse transcriptomique les données sont toujours fournies avec

- 1 ligne = 1 gène `r print(rownames(BIC_expr)[1:4])`
- 1 échantillon biologique = 1 colonne `r print(colnames(BIC_expr)[1:4])`
...
BIC_expr_sample_rows <- t(BIC_expr)
```
...
legend("topleft", legend = rownames(BIC_expr)[1], col = "black", lwd = 2)
```
...
lines(1:200, BIC_expr[10, 1:200], 
     col = "red")
legend("topleft", legend = rownames(BIC_expr)[c(1,10)], col = c("black", "red"), lwd = 2)
```
...
sample_name <- names(BIC_expr[])[1]
sample_values <- unlist(BIC_expr[, sample_name])
...
plot(x, y,
     xlab = paste("Sample:", colnames(BIC_expr)[1]),
     ylab = paste("Sample:", colnames(BIC_expr)[2]),
     panel.first = grid(),
     las = 1)
```
...
pheatmap::pheatmap(BIC_expr[1:200, 1:200], 
...
## ANNE, CETTE FIGURE MANQUE, JE METS TEMPORAIREMENT UN MESSAGE D'ERREUR
# include_graphics(path = "img/cor_vs_euclidian_dist.png")
print("MISSING FIGURE")
```
...
BIC_median <- apply(BIC_expr, 2, median)
BIC_expr_scaled_median <- sweep(x = BIC_expr, MARGIN = 2, STATS = BIC_median)
par(mfrow = c(1,3))
par(mar = c(7, 4.1, 4.1, 1.1)) # adapt margin sizes for the labels
boxplot(BIC_expr[, 1:20], main = "Raw data", las = 2)
boxplot(BIC_expr.centre[, 1:20], main = "Centered on the mean", las = 2)
boxplot(BIC_expr_scaled_median[, 1:20], main = "Centered on the median", las = 2)
...
BIC.sd <- apply(BIC_expr, 2, sd) ## Standard dev per sample
# length(BIC.sd)
mat.BIC.sd <- matrix(rep(BIC.sd, each = nrow(BIC_expr)), ncol = ncol(BIC_expr)) ## Create amatrix with the sd values
#dim(mat.BIC.sd)
#dim(BIC_expr)
colnames(mat.BIC.sd) <- colnames(BIC_expr)
rownames(mat.BIC.sd) <- rownames(BIC_expr)
# View(mat.BIC.sd)
BIC_expr.scaled.sd <- BIC_expr / mat.BIC.sd


## Scaling by sample interquartile range (IQR)
BIC.iqr <- apply(BIC_expr, 2, IQR)
mat.BIC.iqr <- matrix(rep(BIC.iqr, each = nrow(BIC_expr)), ncol = ncol(BIC_expr))
BIC_expr.scaled.iqr <- BIC_expr / mat.BIC.iqr

# range(apply(BIC_expr.scaled.iqr, 2, IQR))
...
boxplot(BIC_expr.scaled.sd[, 1:20], main = "Scaled, standard deviation", las = 2)

## IQR-based scaling
boxplot(BIC_expr.scaled.iqr[, 1:20], main = "Scaled, IQR", las = 2)
par(mar = c(5.1, 4.1, 4.1, 2.1)) # Restore original margin sizes
par(mfrow = c(1,1))
```
...
BIC.iqr.mediane <- apply(BIC_expr.scaled.iqr, 2, median)
BIC_expr.scaled.iqr.mediane <- sweep(BIC_expr.scaled.iqr, 2, BIC.iqr.mediane)
par(mfrow = c(1,3))
...
boxplot(BIC_expr.scaled[, 1:20], main = "Standardized (mean, sd)", las = 2)

## Standardisation based no robust estimators:= median + IQR
boxplot(BIC_expr.scaled.iqr.mediane[, 1:20], main = "Standardized (median, IQR)", las = 2)
par(mar = c(5.1, 4.1, 4.1, 2.1)) # Restore original margin sizes
par(mfrow = c(1,1))
```
...
BIC.scale.euc <- dist(BIC_expr.scaled)
BIC_expr.cor <- cor(t(BIC_expr))
BIC_expr.cor.dist <- 1 - BIC_expr.cor
BIC.scale.hclust <- hclust(BIC.scale.euc)
BIC.scale.max <- dist(BIC_expr.scaled, method = "maximum")
...
wss[1] <- kmeans(BIC_expr.scaled, center = 2)$totss
for (nbgrp in 2:50) {
  wss[nbgrp] <- kmeans(BIC_expr.scaled, center = nbgrp)$tot.withinss
}
...
cluster.kmeans3 <- kmeans(BIC_expr.scaled, center = 3)$cluster
cluster.kmeans3 <- paste0("k", cluster.kmeans3)
cluster.hclust4 <- cutree(hclust(dist(BIC_expr.scaled), method = "ward.D2"), k = 4)
...
levelplot(BIC_expr.cor.dist, xlab = "", ylab = "")
```
