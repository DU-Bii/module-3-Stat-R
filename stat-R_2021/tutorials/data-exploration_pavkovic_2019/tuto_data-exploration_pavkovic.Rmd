...
## Load same data with myEasyLoad
...
## Load proteome data
pfa <- myEasyLoad(file = localPFAfile, rownames.col = 1)
dim(pfa)
...
```{r fa_hist_ugly}
pfaValues <- unlist(as.vector(pfa))
hist(pfaValues)
```
...
```{r fa_hist_breaks}
hist(pfaValues, breaks = 200)
```


```{r fa_hist_log2}
hist(log2(pfaValues), breaks = 100)
```
