...
trans_metadata_file <- download_only_once(
  url_base = pavkovic_base, 
...
## Proteome metadata
prot_metadata_file <- download_only_once(
  url_base = pavkovic_base, 
...
dim(fa)
kable(head(fa), caption = "FA transcriptome loaded with read.delim()")

## Load the same data with load_fix_row_names
fa <- load_fix_row_names(file = local_fa_file, rownames.col = 1)
message("Loaded raw counts of FA transcriptome with load_fix_row_names(): ", nrow(fa), "rows x ", ncol(fa), "columns")
...
# dim(fa_norm)
kable(head(fa), caption = "First loines of the loaded normalised counts of FA transcriptome with load_fix_row_names()")
...
## Raw counts
boxplot(fa, 
        col = transcriptome_metadata$color,
        horizontal = TRUE, 
        las = 1, 
