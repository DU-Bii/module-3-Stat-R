...
sample_stat_prenorm <- data.frame(
  mean = apply(fa_expr_raw, 2, mean, na.rm = FALSE),
  sd = apply(fa_expr_raw, 2, sd, na.rm = FALSE),
  iqr = apply(fa_expr_raw, 2, IQR, na.rm = FALSE),
  Q1 = apply(fa_expr_raw, 2, quantile, p = 0.25, na.rm = FALSE),
  median = apply(fa_expr_raw, 2, median, na.rm = FALSE),
  Q3 = apply(fa_expr_raw, 2, quantile, p = 0.75, na.rm = FALSE),
  max = apply(fa_expr_raw, 2, max, na.rm = FALSE),
  null = apply(fa_expr_raw == 0, 2, sum, na.rm = FALSE)
)
...
## Suppress genes having at least 90% null values
...
## Suppress genes having a mean expression < 10
...
gene_stat_norm <- data.frame(
  mean = apply(fa_expr_log2, 1, mean, na.rm = FALSE),
  var = apply(fa_expr_log2, 1, var, na.rm = FALSE),
  sd = apply(fa_expr_log2, 1, sd, na.rm = FALSE),
  CV = NA,
  min = apply(fa_expr_log2, 1, min, na.rm = FALSE),
  Q1 = apply(fa_expr_log2, 1, quantile, p = 0.25, na.rm = FALSE),
  median = apply(fa_expr_log2, 1, median, na.rm = FALSE),
  Q3 = apply(fa_expr_log2, 1, quantile, p = 0.75, na.rm = FALSE),
  max = apply(fa_expr_log2, 1, max, na.rm = FALSE),
  null = apply(fa_expr_log2 == 0, 1, sum, na.rm = FALSE)
  )
...
gene_stat_norm_annot <- merge(genes, gene_stat_norm, by.x = "ensembl_gene_id", by.y = 0, sort = FALSE)

kable(gene_stat_norm_annot[100:109, ], caption = "Gene-wise statistics after normalisation")
```

**Challenge falcultatif:**

Créez un tableau séparé appelé `gene_stat_norm_sorted` avec les statistiques par gène en les réordonnant par position génomique. Affichez les lignes 5 premières et  5 dernières lignes de ce tableau de statistiques. 

```{r sort_genes_by_chrom}
#### Sorting genes by chromosome ####
message("Sorting genes by chromosome")

order_by_position <- order(gene_stat_norm_annot$chromosome_name, gene_stat_norm_annot$start_position)
gene_stat_norm_sorted <- gene_stat_norm_annot[order_by_position,]

kable(gene_stat_norm_sorted[c(1:5, (nrow(gene_stat_norm_sorted) - 4):nrow(gene_stat_norm_sorted)), ], 
      caption = "Gene-wise statistics after sorting per chromosomal position.")
```
...
print(paste0("Genes with mean > 5 :", sum(high_expression)))
...
print(paste0("Genes with variance > 2 :", sum(high_variance)))
