...
gene_stat_norm <- data.frame(
  mean = apply(fa_expr_log2, 1, mean, na.rm = TRUE),
  var = apply(fa_expr_log2, 1, var, na.rm = TRUE),
  sd = apply(fa_expr_log2, 1, sd, na.rm = TRUE),
  CV = NA,
...
gene_stat_norm_annot <- merge(gene_stat_norm, genes, by.y = "ensembl_gene_id", by.x = 0, sort = FALSE, all.x = TRUE)
# dim(gene_stat_norm_annot)
# names(gene_stat_norm_annot)
# head(gene_stat_norm_annot)
...
Effectuez une analyse d'enrichissement par cluster, à partir de clusters obtenus en coupant l'arbre des gènes (`cutree()`). 

```{r functional_enrichment_clusters, fig.width=7, fig.height=12, out.width="100%", fig.cap="Dynamic map of the enrichment analysis for the clusters obtained from the gene tree. Position the cursor on a circle to see the enriched term. "}
#### Enrichment by cluster ####
message("Enrichment analysis by clusters")

## Get the clusters from the tree
k <- 7 ## Number of clusters
clusters <- cutree(tree = gene_tree_pearson, k = k)

## Convert vector of culsters into a list with one vector of gene IDs for each cluster
cluster_list <- list()
for (c in 1:k) {
  cluster_list[[paste0("cluster_", c)]] <- names(clusters[clusters == c])
}

# View(cluster_list)

## Compute enrichment for each cluster
cluster_enrichment <- gost(query = cluster_list, 
                           organism = "mmusculus", 
                           ordered_query = FALSE, 
                           multi_query = FALSE, 
                           significant = TRUE, 
...
## Check the structure of the result
names(cluster_enrichment)
names(cluster_enrichment$result)
# View(cluster_enrichment$result)
#kable(summary(cluster_enrichment))

# Visualization
gostplot(cluster_enrichment, capped = TRUE, interactive = TRUE)
