...
parameters <- list(
  ...
  test.group = "bdf1_Y187F_Y354F_mutant_4",
  control.group = "Wild_type_4"
)
...
plot(edgeR.DEG.table$PValue, edgeR.DEG.table$FDR, 
     ...
     ylab = parameters$p.adjust.method,
     log = "xy", 
...
legend("topleft",
       legend = c(
         ...
         paste(sep = "", parameters$p.adjust.method, " < ", parameters$alpha, ": ", sum(edgeR.DEG.table$FDR < parameters$alpha), " genes" )))
```
...
par(mfrow = c(2,2))
## MA plot
plotMA(object = results(dds.2groups), las = 1, panel.first = grid(),
